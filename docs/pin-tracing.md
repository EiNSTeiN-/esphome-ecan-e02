...
If the SIT `RS` pin is too difficult to probe directly, use `configs/ecan-e02-can-gpio-probe.yaml`. It pulses ESP32 `GPIO10` low briefly and samples ESP32 `GPIO9` without starting TWAI, so the controller cannot enter bus-off while the physical TX/RX path is checked. Earlier GPIO-probe firmware read `GPIO10` as `tx_gpio=0` even after setting it high, but that probe configured `GPIO10` as output-only. The probe now uses input/output mode so `gpio_get_level(GPIO10)` can be trusted on the next flash.
